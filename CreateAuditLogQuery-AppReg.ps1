...
param (
    ...
    # Pilot mode — restrict audit query to pilot users only
    [bool]$PilotMode = $false,
    [string]$PilotUsersListId = "",   # GUID of the Pilot Users list (required when -PilotMode)
...
# Pilot mode: fetch UPNs from Pilot Users list
$pilotUpns = @()
if ($PilotMode) {
    if ([string]::IsNullOrWhiteSpace($PilotUsersListId)) {
        Write-Error "-PilotUsersListId is required when -PilotMode is enabled."
        exit 1
    }

    Write-Output "Pilot mode enabled — fetching pilot users from list..."
    $uri = "https://graph.microsoft.com/v1.0/sites/$SharePointSiteId/lists/$PilotUsersListId/items?`$expand=fields(`$select=UPN)&`$top=200"
    ...
    if ($pilotUpns.Count -eq 0) {
        Write-Error "Pilot mode enabled but no users found in Pilot Users list."
        exit 1
    }

    Write-Output "Found $($pilotUpns.Count) pilot user(s). Query will be scoped to these users only."
}
...
$query = CreateAuditLogQuery `
    ...
    -userPrincipalNameFilters $pilotUpns
